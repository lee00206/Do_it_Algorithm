...
class Node:
    ...
    def __init__(self, data: Any = None, next: Node = None):
        """초기화"""
        self.data = data    # 데이터 (데이터에 대한 참조: 임의의 형)
        self.next = next    # 뒤쪽 포인터 (뒤쪽 노드에 대한 참조: Node형)
...
class LinkedList:
    ...
    def search(self, data: Any) -> int:
        """data와 같이 같은 노드를 검색"""
        cnt = 0                         # 카운터용 변수
        ptr = self.head                 # 스캔 중인 노드를 참조하기 위한 변수 (head로 초기화)
        while ptr is not None:          # ptr 값이 None이 아니면
            if ptr.data == data:
                self.current = ptr
                return cnt
            cnt += 1
